...
export async function findOrgById(id: string): Promise<Org | null> {
  return await observeSql('orgs.findOrgById', async () => {
    const db = getDb();
...
export async function findOrgBySlug(slug: string): Promise<Org | null> {
  return await observeSql('orgs.findOrgBySlug', async () => {
    const db = getDb();
...
export async function createOrg(input: NewOrg): Promise<Org> {
  return await observeSql('orgs.createOrg', async () => {
    const db = getDb();
...
export async function updateOrg(id: string, patch: Partial<NewOrg>): Promise<Org> {
  return await observeSql('orgs.updateOrg', async () => {
    const db = getDb();
...
export async function deleteOrg(id: string): Promise<void> {
  return await observeSql('orgs.deleteOrg', async () => {
    const db = getDb();
...
export async function slugExists(slug: string): Promise<boolean> {
  return await observeSql('orgs.slugExists', async () => {
    const db = getDb();
    const rows = await db
      .select({ id: orgs.id })
      .from(orgs)
...
export async function findAvailableSlug(base: string): Promise<string> {
  return await observeSql('orgs.findAvailableSlug', async () => {
